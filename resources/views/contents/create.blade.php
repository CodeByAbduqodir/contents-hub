...
<body class="bg-gray-100 font-sans">
    <div class="container mx-auto mt-8 px-4">
        <div class="bg-white p-8 rounded-lg shadow-md border border-[#d4af37]">
            <h1 class="text-2xl font-bold text-gray-800 mb-6">Create New Content</h1>
            ...
            <form action="{{ route('contents.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('title') border-red-500 @enderror" value="{{ old('title') }}">
                    </div>

                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description:</label>
                        <textarea name="description" id="description" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    </div>

                    <div>
                        <label for="url" class="block text-sm font-medium text-gray-700 mb-1">URL (optional):</label>
                        <input type="url" name="url" id="url" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('url') border-red-500 @enderror" value="{{ old('url') }}">
                    </div>
                ...
                <div class="mt-6 flex space-x-2">
                    <button type="submit" class="btn-primary px-4 py-2 rounded-md">Create Content</button>
                    <a href="{{ route('home') }}" class="btn-secondary px-4 py-2 rounded-md">Cancel</a>
                </div>
            </form>
        </div>
    </div>
